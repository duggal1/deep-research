...
export default function DemoResearchPage() {
  ...
  const startResearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/x/crawl?query=${encodeURIComponent(query.trim())}&fullContent=true`);
          ...
          {results.data?.finalAnalysis && (
            <div className="mb-6">
              ...
              <div className="bg-gray-50 p-3 rounded whitespace-pre-wrap">
                {getAnalysisPreview(results.data.finalAnalysis)}
                {results.data.finalAnalysis.length > 300 && (
                  <button 
                    className="text-blue-500 hover:underline"
                    onClick={() => alert(results.data.finalAnalysis)}
                  >
                    View Full Analysis
                  </button>
                )}
                  ...
                  <li key={index} className="border-b pb-2">
                    ...
                    {source.geminiAnalysis && (
                      <div className="mt-1 text-sm">
                        <strong>Analysis:</strong> {getAnalysisPreview(source.geminiAnalysis, 150)}
                      </div>
                    )}
                  </li>
                ))}
